...
func (cs *checkoutService) PlaceOrder(
	...
	for _, it := range prep.orderItems {
		multPrice := money.MultiplySlow(
			*it.Cost,
			uint32(it.GetItem().GetQuantity()),
		)

		total = money.Must(
			money.Sum(total, multPrice),
		)
	}

	txID, err := cs.chargeCard(
		ctx,
		&total,
		req.CreditCard,
	)
	...
	shippingTrackingID, err := cs.shipOrder(
		ctx,
		req.Address,
		prep.cartItems,
	)
	...
	if err := cs.sendOrderConfirmation(
		ctx,
		req.Email,
		orderResult,
	); err != nil {
		log.Warnf(
			"failed to send order confirmation to %q: %+v",
			req.Email,
			err,
		)
	} else {
		log.Infof(
			"order confirmation email sent to %q",
			req.Email,
		)
	}
...
func (cs *checkoutService) prepareOrderItemsAndShippingQuoteFromCart(
	...
	orderItems, err := cs.prepOrderItems(
		ctx,
		cartItems,
		userCurrency,
	)
	...
	shippingUSD, err := cs.quoteShipping(
		ctx,
		address,
		cartItems,
	)
...
func (cs *checkoutService) quoteShipping(
	...
	shippingQuote, err := pb.NewShippingServiceClient(
		cs.shippingSvcConn,
	).GetQuote(
		ctx,
		&pb.GetQuoteRequest{
...
func (cs *checkoutService) getUserCart(
	ctx context.Context,
	userID string,
) ([]*pb.CartItem, error) {
	cart, err := pb.NewCartServiceClient(
		cs.cartSvcConn,
	).GetCart(
		ctx,
		&pb.GetCartRequest{
...
func (cs *checkoutService) emptyUserCart(
	ctx context.Context,
	userID string,
) error {
	if _, err := pb.NewCartServiceClient(
		cs.cartSvcConn,
	).EmptyCart(
		ctx,
		&pb.EmptyCartRequest{
...
func (cs *checkoutService) prepOrderItems(
	...
	cl := pb.NewProductCatalogServiceClient(
		cs.productCatalogSvcConn,
	)

	for i, item := range items {
		product, err := cl.GetProduct(
			ctx,
...
func (cs *checkoutService) convertCurrency(
	...
	result, err := pb.NewCurrencyServiceClient(
		cs.currencySvcConn,
	).Convert(
		context.TODO(),
		&pb.CurrencyConversionRequest{
			From:   from,
			ToCode: toCurrency,
		},
	)
...
func (cs *checkoutService) chargeCard(
	...
	paymentResp, err := pb.NewPaymentServiceClient(
		cs.paymentSvcConn,
	).Charge(
		ctx,
		&pb.ChargeRequest{
...
func (cs *checkoutService) sendOrderConfirmation(
	...
	_, err := pb.NewEmailServiceClient(
		cs.emailSvcConn,
...
func (cs *checkoutService) shipOrder(
	...
	resp, err := pb.NewShippingServiceClient(
		cs.shippingSvcConn,
	).ShipOrder(
		ctx,
		&pb.ShipOrderRequest{
